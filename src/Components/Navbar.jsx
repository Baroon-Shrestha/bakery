              ...
              {navItems.map((item, index) => (
                <Link
                  ...
                  className={`relative px-6 py-3 rounded-full font-medium transition-all duration-300 hover:scale-110 transform ${
                    isScrolled
                      ? "text-gray-700 hover:text-[#009F8A]"
                      : "text-white hover:text-[#009F8A]"
      ...
      >
        ...
        <div
          className={`absolute inset-0 transition-all duration-500 ${
            isMobileMenuOpen
              ? "bg-gradient-to-br from-[#009F8A]/95 via-teal-600/95 to-cyan-600/95 backdrop-blur-xl"
              : "bg-transparent"
          }`}
            ...
            {navItems.map((item, index) => (
              ...
              >
                <Link
                  to={item.to}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="block px-12 py-6 text-4xl md:text-5xl font-bold text-white hover:text-[#009F8A] transition-all duration-300 transform hover:scale-110 active:scale-95 relative group"
                >
                  <span className="relative z-10">{item.label}</span>
